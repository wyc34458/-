<template>
  ...
  export default {
    ...
    methods: {
      created() {
  console.log('Route ID:', this.$route.params.id); // 检查是否能够正确获取到路由参数中的 id 值
  this.id = this.$route.params.id;
      ...
      addItem() {
        ...
        axios.put(`http://localhost:8080/item/update/${this.id}`, postData)
        .then(response => {
        ...
        this.$router.push('/user');
            ...
            this.postError = data && data.message ? data.message : '修改项目失败，请稍后重试。';
            console.error('修改项目失败:', this.postError);
          }
        })
        .catch(error => {
          this.postError = '修改项目失败，请稍后重试。';
